...
namespace Helper.Screen
{
    public static class Pixel
    {
        ...
        public static List<Point> Search(int left, int top, int right, int bottom, Color color, IntPtr hWnd, int variation = 0)
        {
        ...
        public static unsafe Point Search(int left, int top, int right, int bottom, uint color, IntPtr hWnd, int variation = 0)
        {
            ...
            using (var scrnshot = Screenshot(hWnd))
            {
                ...
                var bmpData = scrnshot.LockBits(new Rectangle(0, 0, scrnshot.Width, scrnshot.Height), ImageLockMode.ReadOnly, scrnshot.PixelFormat);
                var pixelSize = ScreenHelper.GetPixelSize(scrnshot.PixelFormat);
                var RGB = ScreenHelper.GetRGB(color);
                    ...
                    for (int y = top; y < bottom; y++)
                    {
                        ...
                        for (int x = left; x < right; x++)
                        {
                            ...
                            if (row[x * pixelSize] <= RGB[2] + variation && row[x * pixelSize] >= RGB[2] - variation &&
                                row[x * pixelSize + 1] <= RGB[1] + variation && row[x * pixelSize + 1] >= RGB[1] - variation &&
                                row[x * pixelSize + 2] <= RGB[0] + variation && row[x * pixelSize + 2] >= RGB[0] - variation)
                            {
                                ret = new Point(x, y);
                                break;
                            }
        ...
        public static Bitmap ScreenshotWithGetPixel(IntPtr hWnd)
        {
            ...
            for (int x = 0; x < width; x++)
                ...
                {
                    bmp.SetPixel(x, y, ScreenHelper.GetColorFromABGR(PInvoke.GetPixel(hdcSrc, x, y)));
                }
            return bmp;
        }
        ...
        {
            return ScreenHelper.GetColorFromABGR(GetPixelColourValue(x, y, hWnd));
        }
