...
namespace Helper.Mouse
{
    public class MouseHook
    {
        ...
        public bool Start()
        {
            ...
            {
                _HookId = PInvoke.SetWindowsHookEx((int)HookType.WH_MOUSE_LL, hookProc, PInvoke.GetModuleHandle(curModule.ModuleName), 0);
            }
            ...
            switch (WinMsg)
            {
                case WindowMessages.WM_LBUTTONDOWN:
                    if (OnMouseLeftButtonDown != null)
                        process = OnMouseLeftButtonDown(this, new MouseDownEventArgs(MouseButtons.Left, info.pt.x, info.pt.y));
                    break;
                case WindowMessages.WM_LBUTTONUP:
                    if (OnMouseLeftButtonUp != null)
                        process = OnMouseLeftButtonUp(this, new MouseUpEventArgs(MouseButtons.Left, info.pt.x, info.pt.y));
                    break;

                case WindowMessages.WM_RBUTTONDOWN:
                    if (OnMouseRightButtonDown != null)
                        process = OnMouseRightButtonDown(this, new MouseDownEventArgs(MouseButtons.Right, info.pt.x, info.pt.y));
                    break;
                case WindowMessages.WM_RBUTTONUP:
                    if (OnMouseRightButtonUp != null)
                        process = OnMouseRightButtonUp(this, new MouseUpEventArgs(MouseButtons.Right, info.pt.x, info.pt.y));
                    break;

                case WindowMessages.WM_MBUTTONDOWN:
                    if (OnMouseMiddleButtonDown != null)
                        process = OnMouseMiddleButtonDown(this, new MouseDownEventArgs(MouseButtons.Middle, info.pt.x, info.pt.y));
                    break;
                case WindowMessages.WM_MBUTTONUP:
                    if (OnMouseMiddleButtonUp != null)
                        process = OnMouseMiddleButtonUp(this, new MouseUpEventArgs(MouseButtons.Middle, info.pt.x, info.pt.y));
                    break;
